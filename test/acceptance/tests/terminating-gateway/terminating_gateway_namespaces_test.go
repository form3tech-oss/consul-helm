...
func TestTerminatingGatewaySingleNamespace(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			k8s.CheckStaticServerConnection(t, nsK8SOptions, true, staticClientName, "http://localhost:1234")
		})
	}
}
...
func TestTerminatingGatewayNamespaceMirroring(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Test that we can make a call to the terminating gateway
			logger.Log(t, "trying calls to terminating gateway")
			k8s.CheckStaticServerConnection(t, ns2K8SOptions, true, staticClientName, "http://localhost:1234")
		})
	}
}
