	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.secure {
				...
				k8s.CheckStaticServerConnection(t, staticClientOpts, false, staticClientName, "http://localhost:1234")
			...
			k8s.CheckStaticServerConnection(t, staticClientOpts, true, staticClientName, "http://localhost:1234")
		})
	}
}
