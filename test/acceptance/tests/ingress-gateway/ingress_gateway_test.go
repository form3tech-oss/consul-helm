	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			if c.secure {
				...
				k8s.CheckStaticServerConnection(t, k8sOptions, false, "static-client", "-H", "Host: static-server.ingress.consul", fmt.Sprintf("http://%s-consul-ingress-gateway:8080/", releaseName))
			...
			k8s.CheckStaticServerConnection(t, k8sOptions, true, "static-client", "-H", "Host: static-server.ingress.consul", fmt.Sprintf("http://%s-consul-ingress-gateway:8080/", releaseName))
		})
	}
}
