	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			if c.secure {
				...
				k8s.CheckStaticServerConnection(t, nsK8SOptions, false, "static-client", "-H", "Host: static-server.ingress.consul", ingressGatewayService)
			...
			k8s.CheckStaticServerConnection(t, nsK8SOptions, true, "static-client", "-H", "Host: static-server.ingress.consul", ingressGatewayService)
		})
	}
}
